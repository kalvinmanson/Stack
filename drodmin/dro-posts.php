...
if($execute == 1) {
	...
	$fq = "";
	$q = "";
	if(isset($_GET['q']) && !empty($_GET['q'])) { $fq = " WHERE dro_posts.name LIKE '%".$_GET['q']."%'"; $q = $_GET['q']; }
	//paginador
	if(isset($_GET['page']) && $_GET['page'] > 1) { $page = $_GET['page'];} else { $page = 1; }
		$paginar = 20;
		$paginaini = ($page - 1) * $paginar;
		$paginall = $m->totalrows('SELECT * FROM dro_posts '.$fq);
		...
		//fin paginador 
	$registros = $m->query("SELECT * FROM dro_posts ".$fq." ORDER BY dro_posts.modified DESC LIMIT ".$paginaini.", ".$paginar);
	...
	$query = sprintf("INSERT INTO dro_posts (name, slug, created, modified) VALUES (%s, %s, %s, %s)",
	...
	$query = sprintf("UPDATE dro_posts SET name=%s, picture=%s, content=%s, modified=%s WHERE id=%s",
	   nosqlinj($_POST['name'], "text"),
	   nosqlinj($_POST['picture'], "text"),
	   nosqlinj($_POST['content'], "text"),
	   nosqlinj(date("Y-m-d H:i:s"), "date"),
	   nosqlinj($registro[0]['dro_posts']['id'], "int"));
...
<form action="" name="editar" id="editar" method="POST">
  <fieldset>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control input-lg" name="name" id="name" required value="<?php echo $registro[0]['dro_posts']['name']; ?>">
    </div>
    <div class="form-group">
      <label for="picture">Imagen (opcional)</label>
      <input type="text" class="form-control ckfile" name="picture" id="picture" value="<?php echo $registro[0]['dro_posts']['picture']; ?>" readonly>
      <?php if(!empty($registro[0]['dro_posts']['picture'])) { ?>
      	<a href="<?php echo $registro[0]['dro_posts']['picture']; ?>" class="btn btn-default fancyb"><i class="glyphicon glyphicon-picture"> </i> Imagen actual: <?php echo $registro[0]['dro_posts']['picture']; ?></a>
      <?php } ?>
    </div>
    <div class="form-group">
      <label for="content">Contenido</label>
      <textarea name="content" class="form-control ckeditor" id="content" rows="10"><?php echo $registro[0]['dro_posts']['content']; ?></textarea>
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" class="table table-hover">
  ...
  foreach($registros as &$registro) { ?>
    <tr>
      ...
      <td><small><?php echo $registro['dro_posts']['created']; ?><br><?php echo $registro['dro_posts']['modified']; ?></small></td>
      <td><?php echo $registro['dro_posts']['name']; ?><br /><small><?php echo $registro['dro_posts']['slug']; ?></small></td>
