...
if($execute == 1) {
	...
	$query = sprintf("INSERT INTO dro_rotas (name, picture, description, orden, link, lang, created, modified) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
	   ...
	   nosqlinj($_POST['description'], "text"),
	...
	$query = sprintf("UPDATE dro_rotas SET name=%s, picture=%s, description=%s, orden=%s, link=%s, lang=%s, modified=%s WHERE id=%s",
	   nosqlinj($_POST['name'], "text"),
	   nosqlinj($nombrefile, "text"),
	   nosqlinj($_POST['description'], "text"),
	   nosqlinj($_POST['orden'], "int"),
...
<form action="" name="editar" id="editar" method="POST" enctype="multipart/form-data">
  <fieldset>
    ...
    <div class="form-group">
      <label for="description">Descripción</label>
      <textarea name="description" class="form-control" id="description"><?php echo $registro[0]['dro_rotas']['description']; ?></textarea>
    </div>
    <div class="form-group">
      <label for="orden">Orden</label>
