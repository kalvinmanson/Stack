...
if($execute == 1) {
	...
	$fq = "";
	$q = "";
	if(isset($_GET['q']) && !empty($_GET['q'])) { $fq = " WHERE dro_cats.name LIKE '%".$_GET['q']."%'"; $q = $_GET['q']; }
	//paginador
	if(isset($_GET['page']) && $_GET['page'] > 1) { $page = $_GET['page'];} else { $page = 1; }
		$paginar = 20;
		$paginaini = ($page - 1) * $paginar;
		$paginall = $m->totalrows('SELECT * FROM dro_cats '.$fq);
		...
		//fin paginador 
	$registros = $m->query("SELECT * FROM dro_cats ".$fq." ORDER BY dro_cats.modified DESC LIMIT ".$paginaini.", ".$paginar);
	...
	$query = sprintf("INSERT INTO dro_cats (name, slug, created, modified) VALUES (%s, %s, %s, %s)",
...
<form action="" name="editar" id="editar" method="POST">
  <fieldset>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control input-lg" name="name" id="name" required value="<?php echo $registro[0]['dro_cats']['name']; ?>">
    </div>
    <div class="form-group">
      <label for="picture">Imagen (opcional)</label>
      <input type="text" class="form-control ckfile" name="picture" id="picture" value="<?php echo $registro[0]['dro_cats']['picture']; ?>" readonly>
      <?php if(!empty($registro[0]['dro_cats']['picture'])) { ?>
      	<a href="<?php echo $registro[0]['dro_cats']['picture']; ?>" class="btn btn-default fancyb"><i class="glyphicon glyphicon-picture"> </i> Imagen actual: <?php echo $registro[0]['dro_cats']['picture']; ?></a>
      <?php } ?>
    </div>
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" class="table table-hover">
  ...
  foreach($registros as &$registro) { ?>
    <tr>
      ...
      <td><small><?php echo $registro['dro_cats']['created']; ?><br><?php echo $registro['dro_cats']['modified']; ?></small></td>
      <td><?php echo $registro['dro_cats']['name']; ?><br /><small><?php echo $registro['dro_cats']['slug']; ?></small></td>
      <td><?php echo $registro['dro_cats']['picture']; ?></td>
      <td width="140">
        <div class="btn-group" role="group">
          <a href="/blog/<?php echo $registro['dro_cats']['slug']; ?>" target="_blank" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-eye-open"></i></a> 
          ...
          <a href="index.php?o=<?php echo $o; ?>&a=dell&id=<?php echo $registro['dro_cats']['id']; ?>" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-remove"></i></a>
        </div>
      </td>
    </tr>
  <?php } ?>
